...
def process_behavior_data(file_path: str, start_time_str: str, injection_time_str: str) -> tuple:
    """
    ...
    post_injection_behavior_df = behavior_df[behavior_df['Time (s)'] >= injection_time].copy()

    post_injection_behavior_df.loc[:, 'Time (s)'] = post_injection_behavior_df['Time (s)'] - injection_time
    baseline_duration = injection_time - start_time
    post_injection_duration = behavior_df['Time (s)'].max() - injection_time

    return baseline_behavior_df, post_injection_behavior_df, baseline_duration, post_injection_duration

def calculate_behavior_frequencies(behavior_df: pd.DataFrame, total_duration: float) -> dict:
    """
    ...
    behavior_frequencies = {behavior: count / total_duration for behavior, count in behavior_counts.items()}
...
def calculate_z_score_differences(baseline_frequencies: dict, post_injection_frequencies: dict) -> dict:
